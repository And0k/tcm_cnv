...
def chars_array_to_datetimeindex(
    ...
    try:
        ...
        if isinstance(date.iat[0] if is_series else date[0], bytes):
            ...
            date = (date.str.decode if is_series else (lambda **kwargs: np.char.decode(date, **kwargs)))(
...
# ----------------------------------------------------------------------
def log_csv_specific_param_operation(key_logged: str, functions_str: Sequence[str], cfg_in) -> None:
    """
...
def param_funs_closure(
    csv_specific_param: Mapping[str, Union[Callable[[str], Any], float]], cfg_in: Mapping[str, Any]
...
def loaded_Idronaut(
    ...
    date = np.column_stack((date["YYYY"], date["-MM-"], date["DD"])).view([("date", "S10"), ("junk", "S2")])[
        "date"
    ]
    date = np.append(date, np.broadcast_to(b"T", date.shape), axis=1).view([("date", "S11"), ("junk", "S9")])[
        "date"
    ]
...
def loaded_Idronaut_terminal(
    ...
    tim = pd.to_datetime(
        a["Time"].str.decode("utf-8", errors="replace"), format="%H:%M:%S.%f"
    ) - pd.Timestamp("1900-01-01")  # default date '1900-01-01' found experimentally!
    date += tim
...
@meta_out_fields(
    keys_del={"Time", "Lat", "Lon"}, add_before={"Time": "M8[ns]"}, add_after={"Lat": "f8", "Lon": "f8"}
)
...
def loaded_sst_mws(
    ...
    if not isinstance(a["Time"][0], bytes):
        ...
        tim = pd.to_datetime(
            a["Time"].str.decode("utf-8", errors="replace"), format="%H:%M:%S"
        ) - pd.Timestamp("1900-01-01")  # default date '1900-01-01' found experimentally!
...
def loaded_sst_old(a: pd.DataFrame, cfg_in: Optional[Mapping[str, Any]] = None) -> pd.Series:
    ...
    if False:  # shifts not works now!
        # Time calc: gets string for time in current zone
        date = np.array(
            a["Date"].values if isinstance(a, pd.DataFrame) else (a["Date"]).astype(object),
            dtype={"DD": ("a2", 0), "MM": ("a2", 3), "YYYY": ("a4", 6)},
        )
        # dtype={'DD': ('a2', 1), 'MM': ('a2', 4), 'YYYY': ('a4', 7)})
        date = (
            date["YYYY"].astype(np.object)
            + b"-"
            + date["MM"].astype(np.object)
            + b"-"
        ...
        date = pd.to_datetime(
            a["Date"].str.decode("utf-8", errors="replace"), format="%d.%m.%Y"
...
def loaded_csv_log(
    ...
    date = np.array(
        a["Time"].astype(object),
        dtype={"DD": ("a2", 0), "MM": ("a2", 3), "YYYY": ("a4", 6), "Time": ("a8", 11)},
    )
    date = (
        date["YYYY"].astype(np.object)
        + b"-"
...
def view_fields(a: Union[pd.DataFrame, np.ndarray], fields) -> Union[pd.DataFrame, np.ndarray]:
    ...
    newdt = np.dtype(dict(names=fields, formats=formats, offsets=offsets, itemsize=itemsize))
    b = a.view(newdt)
    return b
...
def loaded_Baklan(
    ...
    out_len = np.ceil(
        a.shape[0] / cfg_in["decimate_rate"]
    ).astype(
        np.int64
    )  # , dtype=np.uint64, casting='unsafe' - not works. np.floor_divide(a.shape[0], cfg_in['decimate_rate']) + 1 is too small if no reminder
    ...
    for name_out in out_names_need_decimate:
        if name_out in names_in:
            ...
            series = (
                np.ndarray(
                    (len(a), 3),
                    cfg_in["dtype"]["Gx_m/s^2"],
                    a,
                    offset=cfg_in["dtype"].fields["Gx_m/s^2"][1],
                    strides=(a.strides[0], dtype_Gx.itemsize),
                )
                if isinstance(a, np.ndarray)
                else a.loc[:, dtype_Gxyz.names].to_numpy(dtype=dtype_Gx)
            )
            # a.astype(dtype_Gxyz).view(dtype_Gx).reshape(-1, 3) #[['Gx_m/s^2', 'Gy_m/s^2', 'Gz_m/s^2']]

            series = (
                np.sqrt(np.sum(np.square(series), 1)) / 9.8 - 1
            )  # a[['Gx_m/s^2', 'Gy_m/s^2', 'Gz_m/s^2']]/9.8) is not supported
        else:
            continue

        out[name_out] = signal.decimate(
            series, cfg_in["decimate_rate"], ftype="fir", zero_phase=True
        )  # 'fir' becouse of "When using IIR
...
def loaded_ctd_Schuka(
    ...
    date = np.array(date.astype(object), dtype={"MM": ("a2", 0), "DD": ("a2", 3), "YYYY": ("a4", 6)})
    date = (
        date["YYYY"].astype(np.object)
        + b"-"
        + date["MM"].astype(np.object)
...
def loaded_ctd_Schuka_HHMM(
    ...
    date = np.array(date.astype(object), dtype={"DD": ("a2", 0), "MM": ("a2", 3), "YYYY": ("a4", 6)})
    date = (
        date["YYYY"].astype(np.object)
        + b"-"
        + date["MM"].astype(np.object)
...
def day_jumps_correction(cfg_in: Mapping[str, Any], t: Union[np.ndarray, pd.DatetimeIndex]):
    """
    Correction of day jumps in time. Source invalid time might be result as sum of day and time part obtained
    from different not synchronized sources :param cfg_in: dict with field 'time_last' (optional) to check if
    first time values are need to be corrected
    :param t: time
    ...
    )  # .copy() ???, not np.ediff1d because it not allows change type from time to timedelta
    dT_resolution = max(np.timedelta64(1, "s"), np.median(dT)) * 10  # with permissible tolerance
    # correct day jump up
    fun_b_day_jump = lambda dT, dT_day_jump=dT_day_jump, dT_resolution=dT_resolution: np.logical_and(
        dT_day_jump - dT_resolution < dT, dT <= dT_day_jump + dT_resolution
    )
    ...
    if lU or lD:
        ...
        plot_bad_time_in_thread(
            cfg_in, t, np.ones_like(t, np.bool_), None, t_orig, None, "day_jumps_corr", "day jumps correction"
        )
...
loaded_inclin_Baranov = (
    loaded_chain_Baranov  # to find this function by csv_inclin_Baranov.ini config file name
)
        ...
        if cfg_dt:
            ...
            if linear_len:
                ...
                lf.info(
                    ...
                    key,
                    linear_len / dt_full.total_seconds(),
                    time_st,
                    dt0 or "0s",
                )
            else:
                ...
                # 0with this, division before multiplication is needed to not overflow
...
def f_repl_by_dict(
    replist: Iterable[AnyStr], binary_str=True
) -> Callable[[Match[AnyStr]], AnyStr]:  # , repldictvalues={None: b''}
    """
...
def rep_in_file(
    file_in: Union[str, PurePath, BinaryIO, TextIO],
    file_out,
    ...
    lf.warning(
        "preliminary correcting csv file {:s} by removing irregular rows, writing to {}.",
        file_in_path.name,
        file_out,
    )
    ...
    with (
        FakeContextIfOpen(
            lambda x: open(x, **{"mode": "rb"} if binary_mode else {"mode": "r", "errors": "ignore"}), file_in
        ) as fin,
...
def correct_txt(
    ...
    mod_file_name: Callable[[PurePath], PurePath] = lambda n: n.parent.with_name(
    ...
    **kwargs,
    ...
    if file_out:
        ...
        if is_opened:
            ...
            if inf:
                ...
                file_out = mod_file_name(
                    file_in_path.parent / file_in.name
                )  # exclude archive name in out path
    ...
    )  # Windows can not create dir named same as sibling file
    ...
    if sub_str_list:
        fsub = f_repl_by_dict(
            [x if binary_mode else bytes.decode(x) for x in sub_str_list], binary_str=binary_mode
        )
...
def correct_idronaut_terminal_txt(
    ...
    **kwargs,
    ...
    return correct_txt(
        ...
        sub_str_list=[
            br"^(?P<use> *\d{1,4}.\d{1,4}( *[ -]\d{1,4}.\d{1,5}){5,7}( +\d{2}:\d{2}:\d{2}\.\d{2})).*",
            # br'^(?P<use> *\d{1,4}.\d{1,2}( *[ -]\d{1,4}.\d{1,3}){7}( +\d{2}:\d{2}:\d{2}\.\d{2})).*',
            b"^.+",
        ],
        **kwargs,
    )
...
def loaded_nav_supervisor(
    ...
    if not isinstance(a, np.recarray):
        txtT = pd.to_datetime(
            a["Time"].str.decode("utf-8", errors="replace"), format="%H %M %S", errors="coerce"
        ) - pd.to_datetime("00:00:00", format="%H:%M:%S")
        date = (
            pd.to_datetime(
                a["date"].str.decode("utf-8", errors="replace"), format="%y %m %d", errors="coerce"
            )
            + txtT
        )
        names_in = a.columns
        ...
        txtT = np.array(
            a["Time"].astype(object), dtype={"hh": ("a2", 0 + c), "mm": ("a2", 3 + c), "ss": ("a2", 6 + c)}
        )
        bBad = np.int8(date["YY"]) > np.int8(datetime.now().strftime("%y"))
        if np.any(bBad):
            print("Bad date => extract from file name")
            date["YY"], date["MM"], date["DD"] = [
                cfg_in["file_stem"][(slice(k, k + 2))] for k in range(0, 5, 2)
            ]
            # date = np.where(bBad,
        date = np.array(
            century
            + date["YY"].astype(np.object)
            + b"-"
            + date["MM"].astype(np.object)
            + b"-"
            + date["DD"].astype(np.object)
            + b"T"
            + txtT["hh"].astype(np.object)
            + b":"
            + txtT["mm"].astype(np.object)
            + b":"
            + txtT["ss"].astype(np.object),
            "|S19",
            ndmin=1,
        )
        # Bug in supervisor convertor corrupts dates: gets in local time zone.
    ...
    if csv_specific_param is not None:
        if "DepEcho_add" in csv_specific_param:
            a.eval(
                f"DepEcho = DepEcho + {csv_specific_param['DepEcho_add']}", inplace=True
            )  # @corrections['DepEcho_add'] if float
...
def loaded_nav_HYPACK(a: Union[pd.DataFrame, np.ndarray], cfg_in: Mapping[str, Any]) -> pd.DatetimeIndex:
    ...
    if not cfg_in.get("fun_date_from_filename"):
        cfg_in["fun_date_from_filename"] = date_from_filename
    str_date = cfg_in["fun_date_from_filename"](cfg_in["file_stem"], century.decode())
    t = pd.to_datetime(str_date) + pd.to_timedelta(a["Time"].str.decode("utf-8", errors="replace"))
    # t = day_jumps_correction(cfg_in, t.values)
    return t


@meta_out_fields(
    keys_del={"Time", "LatNS", "LonEW", "DatePC", "TimePC"},
...
def loaded_nav_HYPACK_SES2000(
    ...
    tim = pd.to_datetime(
        a.loc[uniq, "Time"].str.decode("utf-8", errors="replace"), format="%H%M%S"
    ) - pd.Timestamp("1900-01-01")  # default date '1900-01-01' found experimentally!
    date += tim
    a["Time"] = pd.NaT  # also changes column's type
...
def loaded_nav_HYPACK_EA600(
    ...
    tim = pd.to_datetime(
        a.loc[uniq, "Time"].str.decode("utf-8", errors="replace"), format="%H%M%S.%f"
    ) - pd.Timestamp("1900-01-01")  # default date '1900-01-01' found experimentally!
    date += tim
    a["Time"] = pd.NaT  # also changes column's type
    a.loc[uniq, "Time"] = day_jumps_correction(cfg_in, date.to_numpy())
...
# @meta_out_fields(keys_del={'Time'}, add_before={'Time': 'M8[ns]'})
def loaded_nav_ADCP_WinRiver2_at(
