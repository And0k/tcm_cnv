...
# from to_pandas_hdf5.csv_specific_proc import rep_in_file, correct_txt, proc_loaded_inclin_Kondrashov
...
def param_funs_closure(
    ...
    """
    Used by proc_loaded_corr(). Converts dict `csv_specific_param` to new dict by removing key suffixes and
...
@meta_out(out_fields)
def proc_loaded_corr(a: Union[pd.DataFrame, np.ndarray],
...
def proc_loaded_inclin_Kondrashov(a: Union[pd.DataFrame, np.ndarray], cfg_in: Mapping[str, Any] = None,
    ...
    if csv_specific_param is not None:
        ...
        if csv_specific_param.get(key):
            ...
            a = a.copy()
            # log_csv_specific_param_operation('proc_loaded_inclin_Kondrashov', csv_specific_param.keys(), cfg_in) mod:
...
def read_csv(paths: Sequence[Union[str, Path]],
    ...
    try:
        try:
            ...
            if cfg_in.get('csv_specific_param'):
                ...
                ddf = ddf.map_partitions(to_pandas_hdf5.csv_specific_proc.proc_loaded_corr,
            ...
            def fun_proc_loaded_and_time_corr_df(df_):
    ...
    if not ddf.known_divisions:  # always
        ...
        # Two to_delayed() leads to call fun_proc_loaded_and_time_corr_df() twice!
...
cfg = {
    'in': {
        'fun_proc_loaded': proc_loaded_inclin_Kondrashov,   # function to convert csv to dask dataframe
    ...
    if cfg['in']['csv_specific_param']:
        ...
        if meta_out is not None:  # only functions with 'meta_out' allowed to modify parameters in same step as loading
            ...
            def fun_proc_loaded_folowed_proc_loaded_corr(a, cfg_in):
                ...
                a = proc_loaded_corr(a, cfg_in, cfg['in']['csv_specific_param'])  # also applies functions defined by strings in csv_specific_param
                return a
